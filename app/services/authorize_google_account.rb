...
module CheckHigh
  ...
  class AuthorizeGoogleAccount
    ...
    def get_id_token_from_google(redirect_uri, code)
      content = HTTP::URI.form_encode({ client_id: @config.GOOGLE_CLIENT_ID,
      ...
      challenge_response =
        HTTP.headers(content_type: 'application/x-www-form-urlencoded')
            .post(@config.GOOGLE_TOKEN_URL, body: content)
    ...
    def get_sso_account_from_api(id_token)
      ...
      response = HTTP.post("#{@config.API_URL}/auth/google_sso", json: signed_sso_info)
